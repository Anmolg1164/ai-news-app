...
import { MapPin, Search, ChevronDown, ArrowUp, Languages, Activity, Cpu, ShieldCheck } from "lucide-react";
...
export default function Home() {
  ...
  // Quota Monitoring (Mock Session Tracking)
  ...
  // Track AI calls (Simplified tracking for UX)
  useEffect(() => {
    const handleAiCall = () => setAiUsage(prev => Math.min(prev + 5, 100));
    window.addEventListener('ai-call', handleAiCall);
    return () => window.removeEventListener('ai-call', handleAiCall);
  }, []);
      ...
      <header className="z-40 glass border-b border-white/40 flex-shrink-0" suppressHydrationWarning>
        <div className="container mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button className="relative group p-2 rounded-xl hover:bg-white/60 transition-all text-muted-foreground">
                  ...
                  {aiUsage > 0 && <span className="absolute top-2 right-2 w-1.5 h-1.5 bg-primary rounded-full" />}
                </button>
              </PopoverTrigger>
              <PopoverContent className="glass border-primary/20 w-64 p-4 z-50">
                <div className="space-y-4">
                  <h4 className="text-[10px] font-bold text-primary uppercase tracking-widest flex items-center gap-2">
                    <Cpu size={12} /> System Quota
                  </h4>
                  <div className="space-y-2">
                    <div className="flex justify-between text-[10px] font-bold">
                      <span className="text-muted-foreground">Gemini Free Tier</span>
                    ...
                    <p className="text-[9px] text-muted-foreground/60 italic leading-relaxed">
                      Resets automatically in <span className="text-primary font-bold">60 seconds</span>. Manual actions (Verify/Briefing) take priority.
                    </p>
                  </div>
                  <div className="pt-2 border-t border-primary/5 flex items-center justify-between">
                    <span className="text-[9px] font-bold uppercase tracking-tighter text-muted-foreground">Serper Search</span>
            ...
            >
              <Activity size={18} className={showSavedOnly ? "text-accent" : "text-primary"} />
            </button>
          </div>
        </div>
      </header>
          ...
          <aside className="lg:col-span-4 space-y-6 overflow-y-auto pr-2 custom-scrollbar hidden lg:block animate-in fade-in slide-in-from-left-10 duration-1000">
            ...
            <div className="p-6 rounded-[2.5rem] glass-dark text-white space-y-4 relative overflow-hidden group hover:glow-secondary transition-all duration-500">
              ...
              <p className="text-sm opacity-80 leading-relaxed font-medium">
                Reading <span className="text-secondary font-bold tracking-tight">{activeCountry.name}</span> reports with <span className="text-secondary font-bold tracking-tight">{activeLanguage.name}</span> AI support.
              </p>
              <div className="space-y-2 pt-2">
                <div className="flex justify-between text-[10px] font-bold uppercase opacity-40">
                  <span>AI Bandwidth</span>
                  <span>{100 - aiUsage}% Free</span>
                </div>
